...
services:
  migrate: { image: "ghcr.io/pr0h0/openmanga-app:0.3.0", build: !reset null }
  api: { image: "ghcr.io/pr0h0/openmanga-app:0.3.0" }
  worker: { image: "ghcr.io/pr0h0/openmanga-app:0.3.0" }
  mock-ai: { image: "ghcr.io/pr0h0/openmanga-app:0.3.0" }
  nginx: { image: "ghcr.io/pr0h0/openmanga-nginx:0.3.0", build: !reset null }
  kokoro: { image: "ghcr.io/pr0h0/openmanga-kokoro:0.3.0", build: !reset null }
```
Then `docker compose pull && docker compose up -d`. Use a version that exists as a release tag, and pin it rather
than `latest` so an upgrade is something you choose (each release also carries its minor tag, here `0.3`). `!reset`
